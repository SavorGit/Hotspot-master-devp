...
public class CoverFlowView extends RelativeLayout {
    ...
    private OnTvBtnCLickListener mOnTvBtnClickListener;
    private OnPhoneBtnClickListener mOnPhoneBtnClickListener;
    private int mChildWidth;
    private int mChildTranslateX;
    ...
    private final int ALPHA_DATUM = 127; // 基础alphaֵ
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            ...
            if (availableHeight < maxChildTotalHeight) {
                ...
                if (mLayoutMode == CoverFlowLayoutMode.MATCH_PARENT) {
                    ...
                    if (heightMode == MeasureSpec.AT_MOST) {
                        ...
                        widthSize = mChildWidth+paddingLeft+paddingRight;
                    }
                }
            }
        } else {
        ...
        if (mGravity == CoverFlowGravity.CENTER_VERTICAL) {// 竖直居中
            ...
            mChildTranslateX = (widthSize >> 1) - (mChildWidth >> 1);
...
//                isOnTopView = false;
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (isOnTopView && touchViewItem == getTopView()
                    ...
                    View phoneView = touchViewItem.findViewById(R.id.btn_phone);
                    View tvView = touchViewItem.findViewById(R.id.btn_tv);

                    Rect rect = new Rect();
                    phoneView.getHitRect(rect);
                    rect.set(rect.left,rect.top+topSpace,rect.right,rect.bottom+topSpace);
                    if(rect.contains((int)event.getX(),(int)event.getY()) && Math.abs(endY-startY)<10) {
                        if (mOnPhoneBtnClickListener != null) {
                            mOnPhoneBtnClickListener.onPhoneBtnClick(getTopViewPosition(), getTopView());
                        }
                    }

                   tvView.getHitRect(rect);
                    rect.set(rect.left,rect.top+topSpace,rect.right,rect.bottom+topSpace);
                    if(rect.contains((int)event.getX(),(int)event.getY()) && Math.abs(endY-startY)<10) {
                        if (mOnTvBtnClickListener != null) {
                            mOnTvBtnClickListener.onTvBtnClick(getTopViewPosition(), getTopView());
                        }
                    }
                }
                touchViewItem = null;
    ...
    public interface OnTopViewClickListener extends OnPhoneBtnClickListener {

        void onClick(int position, View itemView);

        @Override
        void onPhoneBtnClick(int position, View itemView);
    }

    public void setOnTvBtnClickListener(OnTvBtnCLickListener listener) {
        this.mOnTvBtnClickListener = listener;
    }

    public void setOnPhoneBtnClickListener(OnPhoneBtnClickListener listener) {
        this.mOnPhoneBtnClickListener = listener;
    }

    public interface OnTvBtnCLickListener {

        void onTvBtnClick(int position, View itemView);
    ...
    private void endAnimation() {
        if (mAnimationRunnable != null) {
            ...
            if(mOffset>mStartOffset) {
                RecordUtils.onEvent(getContext(),R.string.home_demand_slide_up);
            }else {
                RecordUtils.onEvent(getContext(),R.string.home_demand_slide_down);
            }
        }
    }

    private void touchMoved(MotionEvent event) {
        float pos = (event.getY() / mHeight) * MOVE_POS_MULTIPLE - 5;
        pos /= 2;
...
//        mVelocity.addMovement(event);
    ...
    private void touchEnded(MotionEvent event) {
        ...
        if (mTouchMoved || (mOffset - Math.floor(mOffset)) != 0) {
            ...
            float speed = mVelocity.getYVelocity()/2;

            speed = (speed / mHeight) * MOVE_SPEED_MULTIPLE;
            if (speed > MAX_SPEED)
                speed = MAX_SPEED;
...
//        }
     ...
     * 翻到前页
     */
    public void gotoPreviousNoAnimator() {
        doAnimator(0,-1.0f);
    }
    ...
    public void doAnimator(long time ,float target) {
    ...
    public void donotAnimator(float target) {
        if (isOnAnimator || onTouchMove) { //如果正在执行点击切换动画 或者 正在执行触摸移动
            return;
        }
        isOnAnimator = true;
