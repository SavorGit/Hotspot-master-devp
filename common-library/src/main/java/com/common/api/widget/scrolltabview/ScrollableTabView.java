    ...
    public void selectTab(int position) {
        ...
        for (int i = 0, pos = 0; i < mContainer.getChildCount(); i ++ , pos++) {
            ...
            if(tab instanceof Button) {
            	((Button) tab).setTextColor(getContext().getResources().getColor(R.color.tab_text_default));;
            }
            tab.setSelected(pos == position);
        }
        
        View selectedTab = mContainer.getChildAt(position);
        if(selectedTab instanceof Button) {
        	((Button) selectedTab).setTextColor(getContext().getResources().getColor(R.color.tab_text_color));
        }
