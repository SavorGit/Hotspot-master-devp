...
public class PullToRefreshExpandableListView extends PullToRefreshAdapterViewBase<ExpandableListView> {
	...
	private void initLoadMore(Context context) {
		...
		footerTv = (TextView) mFooterView.findViewById(R.id.pull_to_loadmore_text);
		progressView = mFooterView.findViewById(R.id.pull_to_loadmore_progress);
	}

	private void updateUI(boolean hasMore) {
		if(mRefreshableView.getFooterViewsCount() < 1) {
			mRefreshableView.addFooterView(mFooterView, null, false);
		}
		if (hasMore) {
			progressView.setVisibility(View.VISIBLE);
			footerTv.setText(R.string.pull_to_refresh_loadmore);
		} else {
			progressView.setVisibility(View.GONE);
			footerTv.setText(R.string.pull_to_refresh_all);
		}
	}
